...
- Exposed click radius and move distance state, `getViewWidget()`, `prepareForPaint()`, and add/remove wrappers that emit item notifications.
...
- `addItem(QGraphicsItem*)` and `removeItem(QGraphicsItem*)` intentionally hide the same-signature Qt methods rather than override them; Qt's methods are not virtual, so notifications are emitted when callers use the concrete `GraphicsScene` API.
...
- `python3 -m pytest -q` — exit 0; 208 passed in 32.49s.
