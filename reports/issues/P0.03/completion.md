...
Original gate finding: Pi completed but left no git changes. That bounded rework added the issue-owned dashboard command, focused tests, generated dashboard artifact, and completion report. Current autoreview rework fixes stale manifest trust by deriving every row's expected `status`/`completion` from target path presence before dashboard counts, `--check`, or `--require-complete` can pass. No commits, pushes, merges, workflow-policy edits, or scope expansion were performed.
...
- Every row's `status`/`completion` metadata must match its required `target_header_path`, `target_source_path`, or `target_path` entries on disk before dashboard counts or completion gates can pass.
...
- stale incomplete target metadata without writes;
...
- Failing-fixture cases: `test_P0_03_check_rejects_stale_incomplete_target_metadata_without_writes` keeps fixture rows marked partial/missing, creates their remaining target files, and verifies `--check` fails read-only; `test_P0_03_require_complete_rejects_complete_rows_with_missing_targets` marks every fixture row complete, deletes `src/pyqtgraph/PlotData.cpp`, and verifies `--require-complete` fails on stale target metadata.
...
- `python3 -m pytest -q tests -k P0_03` exited 0: `10 passed, 295 deselected in 0.82s`.
- `python3 -m pytest -q tests/oracle/test_P0_03_status_dashboard.py` exited 0: `10 passed in 1.30s`.
- `python3 -m pytest -q tests/oracle/test_P0_03_status_dashboard.py::test_P0_03_check_rejects_stale_incomplete_target_metadata_without_writes tests/oracle/test_P0_03_status_dashboard.py::test_P0_03_require_complete_rejects_complete_rows_with_missing_targets` exited 0: `2 passed in 0.17s`.
...
Ownership result: passed; no branch-diff path requires issue scope expansion. Current follow-up rework paths are `scripts/summarize_status`, `tests/oracle/test_P0_03_status_dashboard.py`, and `reports/issues/P0.03/completion.md`, all within issue-owned paths or focused adjuncts.
