...
- Upstream zero-length `Point.norm()` raises `ZeroDivisionError`; existing C++ returns NaN components for `Point().norm()`. P2.01 records the upstream behavior in the fixture and keeps the existing C++ semantic under the approved minimal implementation scope.
