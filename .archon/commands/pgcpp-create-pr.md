---
description: Create or update a review-ready PR for a validated pgcpp issue branch.
argument-hint: (reads workflow artifacts)
---

# pgcpp create PR

Create or update a review-ready pull request for the current branch. This command may commit and push the PR branch. It must never push to `main`, merge, or alter `MISSION.md`/`FACTORY_RULES.md` content.
...
5. Create or update a PR against `main` and mark it ready for review (`gh pr ready`) before returning; `pgcpp-validate-pr` rejects draft PRs.
