...
inline void PColorMeshItem::setColorMap(const pyqtgraph::ColorMap& colorMap)
{
    const auto table = colorMap.getLookupTable(0.0, 1.0, 256, true, pyqtgraph::ColorMap::OutputMode::QColor);
...
{
    if (const std::optional<pyqtgraph::ColorMap> viridis = pyqtgraph::get(QStringLiteral("viridis")); viridis.has_value()) {
        return viridis->getLookupTable(0.0, 1.0, 256, true, pyqtgraph::ColorMap::OutputMode::QColor).colors;
    }
    return pyqtgraph::ColorMap({0.0, 0.25, 0.5, 0.75, 1.0},
        {QColor(68, 1, 84), QColor(59, 82, 139), QColor(33, 145, 140), QColor(94, 201, 98), QColor(253, 231, 37)},
        QStringLiteral("viridis-fallback"))
        .getLookupTable(0.0, 1.0, 256, true, pyqtgraph::ColorMap::OutputMode::QColor)
        .colors;
}
