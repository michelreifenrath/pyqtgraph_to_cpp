...
struct ColorMapMenuActionData {
    QString name;
    QString source;
    std::optional<pyqtgraph::ColorMap> embeddedMap;
};

struct ColorMapMenuSpecifier {
    QString name;
    std::optional<QString> source;
    std::optional<pyqtgraph::ColorMap> map;
};
...
public:
    ...
    [[nodiscard]] static pyqtgraph::ColorMap actionDataToColorMap(const ColorMapMenuActionData& data);
...
signals:
    void sigColorMapTriggered(const pyqtgraph::ColorMap& colorMap);
...
Q_DECLARE_METATYPE(pyqtgraph::widgets::ColorMapMenuActionData)
