...
#include "../../../include/cppqtgraph/widgets/ColorButton.hpp"
...
{
    return cppqtgraph::mkColor(color);
}
...
QColor normalizeColor(std::initializer_list<int> color)
{
    if (color.size() == 3) {
        ...
        return cppqtgraph::mkColor(static_cast<double>(it[0]), static_cast<double>(it[1]), static_cast<double>(it[2]));
    }
    if (color.size() == 4) {
        const auto it = color.begin();
        return cppqtgraph::mkColor(static_cast<double>(it[0]),
...
void ColorButton::paintEvent(QPaintEvent* event)
{
    ...
    painter.drawRect(rect);
    painter.setBrush(cppqtgraph::mkBrush(color_));
    painter.drawRect(rect);
}
