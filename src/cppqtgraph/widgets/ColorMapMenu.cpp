...
#include "../../../include/cppqtgraph/widgets/ColorMapMenu.hpp"
...
QWidget* buildMenuEntryWidget(const cppqtgraph::ColorMap& colorMap, const QString& text)
{
    const auto lut = colorMap.getLookupTable(0.0, 1.0, 32, true, cppqtgraph::ColorMap::OutputMode::Byte);
...
cppqtgraph::ColorMap resolveColorMap(const QString& name, const QString& source, const std::optional<cppqtgraph::ColorMap>& embeddedMap)
{
    ...
    if (const auto resolved = cppqtgraph::get(name, source)) {
        return *resolved;
    }
...
cppqtgraph::ColorMap ColorMapMenu::actionDataToColorMap(const ColorMapMenuActionData& data)
{
...
void ColorMapMenu::populateLocalSubMenu(QMenu* menu)
{
    ...
    QStringList names;
    for (const QString& name : cppqtgraph::listMaps()) {
...
void ColorMapMenu::addMenuEntryAction(QMenu* menu, const QString& name, const ColorMapMenuActionData& data)
{
    ...
    const cppqtgraph::ColorMap previewMap = actionDataToColorMap(data);
