...
#include "../../../include/cppqtgraph/widgets/ColorMapButton.hpp"
...
void ColorMapButton::setColorMap(const cppqtgraph::ColorMap& colorMap)
{
...
{
    if (const auto resolved = cppqtgraph::get(name)) {
...
cppqtgraph::ColorMap ColorMapButton::colorMap() const
{
    return colorMap_;
}
...
ColorMapMenu* ColorMapButton::getMenu()
{
    if (menu_ == nullptr) {
        ...
        connect(menu_, &ColorMapMenu::sigColorMapTriggered, this, [this](const cppqtgraph::ColorMap& colorMap) {
...
void ColorMapButton::setColorMapInternal(const cppqtgraph::ColorMap& colorMap, bool emitSignal)
{
    colorMap_ = colorMap;
    cachedImage_ = QImage();
    if (emitSignal) {
