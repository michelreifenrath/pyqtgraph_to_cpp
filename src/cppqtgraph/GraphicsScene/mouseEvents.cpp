...
cppqtgraph::Point toPoint(const QPointF& point)
{
    return cppqtgraph::Point(point);
}

cppqtgraph::Point toPoint(const QPoint& point)
{
    return cppqtgraph::Point(point);
}

cppqtgraph::Point mapFromScene(QGraphicsItem* item, const cppqtgraph::Point& scenePos)
{
    if (item == nullptr) {
        return scenePos;
    }
    return cppqtgraph::Point(item->mapFromScene(scenePos));
}
...
void copyButtonDownPositions(QGraphicsSceneMouseEvent* moveEvent,
    QHash<Qt::MouseButton, cppqtgraph::Point>& scenePositions,
    QHash<Qt::MouseButton, cppqtgraph::Point>& screenPositions)
{
...
void insertButtonDownPosition(Qt::MouseButton button,
    const cppqtgraph::Point& scenePos,
    const cppqtgraph::Point& screenPos,
    QHash<Qt::MouseButton, cppqtgraph::Point>& scenePositions,
    QHash<Qt::MouseButton, cppqtgraph::Point>& screenPositions)
{
...
cppqtgraph::Point pointForButton(
    const QHash<Qt::MouseButton, cppqtgraph::Point>& positions,
...
MouseDragEvent::MouseDragEvent(QGraphicsSceneMouseEvent* moveEvent, QGraphicsSceneMouseEvent* pressEvent,
    ...
    : scenePos_(moveEvent != nullptr ? toPoint(moveEvent->scenePos()) : cppqtgraph::Point())
    , screenPos_(moveEvent != nullptr ? toPoint(moveEvent->screenPos()) : cppqtgraph::Point())
...
cppqtgraph::Point MouseDragEvent::scenePos() const { return scenePos_; }

cppqtgraph::Point MouseDragEvent::screenPos() const { return screenPos_; }

cppqtgraph::Point MouseDragEvent::buttonDownScenePos() const { return buttonDownScenePos(button_); }

cppqtgraph::Point MouseDragEvent::buttonDownScenePos(Qt::MouseButton button) const
{
    return pointForButton(buttonDownScenePos_, button, button_);
}

cppqtgraph::Point MouseDragEvent::buttonDownScreenPos() const { return buttonDownScreenPos(button_); }

cppqtgraph::Point MouseDragEvent::buttonDownScreenPos(Qt::MouseButton button) const
{
    return pointForButton(buttonDownScreenPos_, button, button_);
}

cppqtgraph::Point MouseDragEvent::lastScenePos() const { return lastScenePos_; }

cppqtgraph::Point MouseDragEvent::lastScreenPos() const { return lastScreenPos_; }

cppqtgraph::Point MouseDragEvent::pos() const { return mapFromScene(currentItem_, scenePos_); }

cppqtgraph::Point MouseDragEvent::lastPos() const { return mapFromScene(currentItem_, lastScenePos_); }

cppqtgraph::Point MouseDragEvent::buttonDownPos() const { return buttonDownPos(button_); }

cppqtgraph::Point MouseDragEvent::buttonDownPos(Qt::MouseButton button) const
{
...
MouseClickEvent::MouseClickEvent(QGraphicsSceneMouseEvent* pressEvent, bool doubleClick)
    : scenePos_(pressEvent != nullptr ? toPoint(pressEvent->scenePos()) : cppqtgraph::Point())
    , screenPos_(pressEvent != nullptr ? toPoint(pressEvent->screenPos()) : cppqtgraph::Point())
...
cppqtgraph::Point MouseClickEvent::scenePos() const { return scenePos_; }

cppqtgraph::Point MouseClickEvent::screenPos() const { return screenPos_; }

cppqtgraph::Point MouseClickEvent::pos() const { return mapFromScene(currentItem_, scenePos_); }

cppqtgraph::Point MouseClickEvent::lastPos() const { return pos(); }
...
HoverEvent::HoverEvent(QGraphicsSceneMouseEvent* moveEvent, bool acceptable)
    : scenePos_(moveEvent != nullptr ? toPoint(moveEvent->scenePos()) : cppqtgraph::Point())
    , screenPos_(moveEvent != nullptr ? toPoint(moveEvent->screenPos()) : cppqtgraph::Point())
    , lastScenePos_(moveEvent != nullptr ? toPoint(moveEvent->lastScenePos()) : cppqtgraph::Point())
    , lastScreenPos_(moveEvent != nullptr ? toPoint(moveEvent->lastScreenPos()) : cppqtgraph::Point())
...
cppqtgraph::Point HoverEvent::scenePos() const { return scenePos_; }

cppqtgraph::Point HoverEvent::screenPos() const { return screenPos_; }

cppqtgraph::Point HoverEvent::lastScenePos() const { return lastScenePos_; }

cppqtgraph::Point HoverEvent::lastScreenPos() const { return lastScreenPos_; }

cppqtgraph::Point HoverEvent::pos() const { return mapFromScene(currentItem_, scenePos_); }

cppqtgraph::Point HoverEvent::lastPos() const { return mapFromScene(currentItem_, lastScenePos_); }
